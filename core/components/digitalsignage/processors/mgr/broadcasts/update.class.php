...
class DigitalSignageBroadcastUpdateProcessor extends modObjectUpdateProcessor
{
    /**
     * @access public.
     * @var String.
    ...
    public function beforeSave()
    {
        ...
        if ($exists >= 1) {
            ...
            $resourceResponse = $this->modx->runProcessor('resource/update', [
            ...
            if ($resourceResponse->isError()) {
                ...
                foreach (array_filter(explode(',', $this->getProperty('ticker_url'))) as $ticker) {
                    if (!preg_match('/^(http|https)/i', $ticker)) {
...
return 'DigitalSignageBroadcastUpdateProcessor';
