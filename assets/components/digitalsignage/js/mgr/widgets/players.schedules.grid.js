DigitalSignage.grid.PlayerSchedules = function(config) {
    ...
    Ext.applyIf(config, {
        ...
        fields      : ['id', 'player_id', 'broadcast_id', 'description', 'type', 'start_time', 'start_date', 'end_time', 'end_date', 'day', 'entire_day', 'type_formatted', 'date_formatted', 'broadcast_color', 'resource_pagetitle'],
        paging      : true,
        pageSize    : 7,
        showPerPage : false,
        sortBy      : 'id',
...
Ext.extend(DigitalSignage.grid.PlayerSchedules, MODx.grid.Grid, {
    ...
    getMenu: function() {
        return [{
            text    : '<i class="x-menu-item-icon icon icon-edit"></i>' +_('digitalsignage.player_schedule_update'),
            handler : this.updateSchedule,
            scope   : this
        }, '-', {
            text    : '<i class="x-menu-item-icon icon icon-times"></i>' +_('digitalsignage.player_schedule_remove'),
    ...
    renderDate: function(d, c, e) {
        return String.format('<span class="icon icon-{0}"></span> {1}', e.data.type === 'day' ? 'thumb-tack' : 'clock-o', d);
    },
    renderBroadcast: function(d, c, e) {
        return String.format('<span class="x-cal-combo x-cal-{0}"><span class="ext-cal-picker-icon"></span></span> {1}', e.json.broadcast_color, e.json.resource_pagetitle);
    }
});
