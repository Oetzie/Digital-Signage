...
;(function($, window, document, undefined) {
    ...
    SlideDumpert.Defaults = {
        ...
        top5                : 'https://api-live.dumpert.nl/mobile_api/json/top5/dag/{date}',
        top5Type            : 'JSON',

        randomVideo         : 'https://api-live.dumpert.nl/mobile_api/json/latest/{page}?tag=fail',
                            ...
                            if (result.responseJSON) {
                                if (result.responseJSON.items.length > 0) {
                                    ...
                                    for (var i = 0; i < result.responseJSON.items.length; i++) {
                                        ...
                                        if (item.thumbnail) {
                                            item.thumbnail = '/assets/components/digitalsignage/connector.php?action=web/media&url=' + item.thumbnail;
                                        }
                                                    ...
                                                    for (var iii = 0; iii < media.variants.length; iii++) {
                                                        if (media.variants[iii].version === '720p') {
                                                            data = {
                                                                ...
                                                                date        : item.date,
                                                                stats       : item.stats,
                                                                media       : '/assets/components/digitalsignage/connector.php?action=web/media&url=' + media.variants[iii].uri
                                                            };
                                                        }
                                                    }
                                                }
